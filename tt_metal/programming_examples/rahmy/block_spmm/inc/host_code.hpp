...
// TODO: figure out the include for WHCC
...
void bsr_spmm_multicore_sparse_mcast(
    ...
    uint32_t indexing_data_single_tile_size = tile_size(indexing_data_format);
   ...
   std::deque<uint32_t> folded_bsr_matrix_indices;
    ...
    tt::tt_metal::distributed::ReplicatedBufferConfig buffer_config_dst{.size = dram_buffer_dst_total_size};
    tt::tt_metal::distributed::ReplicatedBufferConfig buffer_config_A{.size = dram_buffer_A_size};
    tt::tt_metal::distributed::ReplicatedBufferConfig buffer_config_B{.size = dram_buffer_B_size};
    tt::tt_metal::distributed::ReplicatedBufferConfig buffer_config_col_indices{.size = dram_buffer_col_indices_size};
    tt::tt_metal::distributed::ReplicatedBufferConfig buffer_config_indptr{.size = dram_buffer_indptr_size};
    distributed::DeviceLocalBufferConfig dram_config{
        .page_size = single_tile_size, .buffer_type = tt_metal::BufferType::DRAM};

    auto dst_dram_buffer = tt::tt_metal::distributed::MeshBuffer::create(buffer_config_dst, dram_config, mesh_device.get());
    auto src0_dram_buffer = tt::tt_metal::distributed::MeshBuffer::create(buffer_config_A, dram_config, mesh_device.get());
    auto src1_dram_buffer = tt::tt_metal::distributed::MeshBuffer::create(buffer_config_B, dram_config, mesh_device.get());
    auto column_indices_dram_buffer = tt::tt_metal::distributed::MeshBuffer::create(buffer_config_col_indices, dram_config, mesh_device.get());
    auto indptr_dram_buffer = tt::tt_metal::distributed::MeshBuffer::create(buffer_config_indptr, dram_config, mesh_device.get());
    ...
    */
    ...
    bool col_indices_is_dram = true;
    bool indptr_is_dram = true;
    ...
    // New api, TensorAccessor args
    TensorAccessorArgs(*src0_dram_buffer).append_to(reader_compile_time_args);
    TensorAccessorArgs(*src1_dram_buffer).append_to(reader_compile_time_args);
    TensorAccessorArgs(*column_indices_dram_buffer).append_to(reader_compile_time_args);
    TensorAccessorArgs(*indptr_dram_buffer).append_to(reader_compile_time_args);
    ...
    // EnqueueWriteBuffers
    tt::tt_metal::distributed::EnqueueWriteMeshBuffer(cq, src0_dram_buffer, a.data, true);
    tt::tt_metal::distributed::EnqueueWriteMeshBuffer(cq, src1_dram_buffer, b.data, true);
    tt::tt_metal::distributed::EnqueueWriteMeshBuffer(cq, column_indices_dram_buffer, a.indices, true);
    tt::tt_metal::distributed::EnqueueWriteMeshBuffer(cq, indptr_dram_buffer, a.indptr, true);
    ...
    // TODO: wait for TT team to figure out subbuffers in the new api

    // tt::tt_metal::distributed::EnqueueReadMeshBuffer(cq, dst_dram_buffer, output.data, true);
    // cq.enqueue_read_mesh_buffer(output.data.data(), dst_dram_buffer, true); // if this works, most of the examples will fail due to no filling of zeros :/
    
    // TODO: read to a temp vector, then copy regions of the temp vector to output
    std::vector<bfloat16> temp;
    distributed::EnqueueReadMeshBuffer(cq, temp, dst_dram_buffer, true);
    ...
    for (size_t row_index = 0; row_index < a.indptr.size() - 1; row_index++) {
        ...
        // std::copy(temp.begin() + nonzero_row_index * R * N, temp.begin() + nonzero_row_index * (R * N + 1), output.data.begin() + row_index * R * N);
        for (int i = 0; i < R * N; i++){
            output.data[row_index * R * N + i] = temp[nonzero_row_index * R * N + i];
        }
        // tt::tt_metal::distributed::EnqueueReadSubBuffer(cq, dst_dram_buffer, output.data.data() + (row_index * R * N), DRAM_row, true);
        nonzero_row_index++;
    }
    ...
    //         continue;
    //     BufferRegion DRAM_row(nonzero_row_index * dram_buffer_dst_row_size, dram_buffer_dst_row_size);
    //     std::vector<distributed::MeshCommandQueue::ShardDataTransfer> transfers = {{
    //         .shard_coord = tt::tt_fabric::MeshCoordinate(0, 0), // single chip, so the mesh coord is always 0, 0
    //         .host_data = output.data.data() + (row_index * R * N),
    //         .region = DRAM_row
    //     }};
    //     cq.enqueue_read_shards(transfers, dst_dram_buffer, true);
    //     // tt::tt_metal::distributed::EnqueueReadSubBuffer(cq, dst_dram_buffer, output.data.data() + (row_index * R * N), DRAM_row, true);
    //     nonzero_row_index++;
    // }
...
//     uint32_t col_indices_single_tile_size = tile_size(col_indices_data_format);
