...
public:
    ...
    bsr_matrix(dense_matrix<T>& other, uint32_t right_outer_dim = 0) :
        ...
        uint32_t Nt = right_outer_dim > 0 ? right_outer_dim / TILE_SIZE : std::max(H, W) / TILE_SIZE;
        auto matmul_params = bmm_op_utils::get_large_matmul_params(H / TILE_SIZE, Nt, 8, 8, 2);
        ...
        R = per_core_M * TILE_SIZE;
        C = std::min(per_core_N * TILE_SIZE, (uint32_t) 2 * TILE_SIZE);
        // if (W % 64 == 0)
        //     C = 64;
        // if (H % 64 == 0)
        //     R = 64; /
